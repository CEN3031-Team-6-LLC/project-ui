...
import InputField from "../GeneralFire/InputField";
import { validatePlumeFields } from "../GeneralFire/helpers";
...
import Nuclide from "../GeneralFire/Nuclide";
import LungClass from "../GeneralFire/LungClass";
...
const GeneralPlume = props => {
  ...
  const [fieldValues, setFieldValues] = React.useState({
    ...
    releaseHeight: { error: false, value: "" },
  ...
  return (
    <div className={classes.generalPlume} hidden={hidden}>
      ...
      <InputField
        ...
        unitTogglelable={true}
        type="number"
        errorMessage="Error: Release Height must be greater than 0"
        unit={fieldValues.unitSystem === "metric" ? "m" : "ft"}
        inputValidation={value => value >= 0}
        setUnit={toggleUnit}
        onChange={val => {
          setFieldValues({ ...fieldValues, releaseHeight: { ...val } });
        }}
      />
...
<div className={classes.icrpContainer}>
      ...
      <MaterialUI.Button
        variant="contained"
        onClick={() => {
          const valid = validatePlumeFields(fieldValues);
          if (valid === true) {
            onPlumeClick(fieldValues);
          } else {
            setError({
              status: true,
